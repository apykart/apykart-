...
export class ProfilePage {
  ...
  render() {
    ...
    const ordersCount = 0; // would be fetched from orders listener
      ...
      <div class="profile-page">
        <div class="profile-header">
          ...
          <div class="profile-info">
            ...
            <div>${profile.email || currentUser.email}</div>
          </div>
          <button class="logout-btn">Sign Out</button>
        </div>
        <div class="stats-row">
          <div class="stat"><span>${ordersCount}</span>Orders</div>
          <div class="stat"><span>${coins}</span>Coins</div>
          <div class="stat"><span>${userData.wishlist?.length || 0}</span>Wishlist</div>
        </div>
        <div class="menu">
  ...
  renderGuest() {
    this.container.innerHTML = `
      <div class="profile-page guest">
        <div class="guest-card">
          <h2>Welcome to Apykart!</h2>
          <p>Sign in to access your orders, coins, and more.</p>
          <button class="google-signin-btn">Sign in with Google</button>
        </div>
      </div>
    `;
    this.container.querySelector('.google-signin-btn').addEventListener('click', async () => {
