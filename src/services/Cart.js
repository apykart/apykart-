...
export function getCart() { return userData.cart || []; }
...
export async function addToCart(productId, qty = 1) {
  ...
  else userData.cart.push({ id: productId, qty });
  await saveCart();
}
export async function updateCartItemQty(productId, delta) {
  ...
  if (item) {
    item.qty += delta;
    if (item.qty <= 0) userData.cart = userData.cart.filter(i => i.id !== productId);
    await saveCart();
  }
}
...
async function saveCart() {
  ...
  await updateDoc(doc(db, 'users', currentUser.uid), { cart: userData.cart });
}
